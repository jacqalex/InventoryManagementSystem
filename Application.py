...
from flask_migrate import Migrate #This helps in migrating a DB
...
def ItemsCategory():
    ...
    for itm in catList:
        tabl = tabl+"<TR ><td></td><TD>" + \
            str(itm.ItemCatID)+"</TD><TD>"+itm.ItemCatDesc+"</TD></TR>"
    tabl = tabl+"</TABLE></div>"
...
def Items():
    ...
    for itm in itmList:
        tabl = tabl+"<TR ><td></td><TD>" + \
            str(itm.ItemID)+"</TD><TD>"+itm.ItemDesc + \
            "</TD><TD>"+itm.itemCat.ItemCatDesc+"</TD></TR>"
    tabl = tabl+"</TABLE></div>"
    catList = ItemCategory.query.all()
    ...
    for cat in catList:
        dd = dd+"<option value=\"" + \
            str(cat.ItemCatID)+"\">"+cat.ItemCatDesc + "</option>"
    dd = dd + '''</select>
</div>'''
...
def AddItems():
    ...
    for itm in itmList:
        tabl = tabl+"<TR ><td></td><TD>"+str(itm.ItemID)+"</TD><TD>"+itm.ItemDesc + \
            "</TD><TD>"+itm.itemCat.ItemCatDesc + \
            "</TD><TD>"+str(itm.AvailAmt)+"</TD></TR>"
    tabl = tabl+"</TABLE></div>"
    dd = '''<div class="form-group">
            <select class="form-control" name="ItemID">'''
    for itm in itmList:
        dd = dd+"<option value=\"" + \
            str(itm.ItemID)+"\">"+itm.ItemDesc + "</option>"
    dd = dd + '''</select>
</div>'''
...
def itemIssue():
    ...
    strg = '''<Form><div class="form-group">
            ...
            <TD>Issue</TD></TH>'''
    for itm in itmList:
        strg = strg+"<TR ><td></td><TD>" + \
            str(itm.item.ItemDesc)+"</TD><TD>"+str(itm.ReqDate) + \
            "</TD><TD>"+str(itm.ReqQty)+"</TD>"
        strg = strg+"<TD>"+str(itm.user.userName)+"</TD><TD>"
        strg = strg+"<div class=\"col-12\"><a href=" + \
            url_for('issUpdt', tid=itm.reqID)+">Issue</a></div></TD></TR>"
...
def itemReturn():
    ...
    for itm in itmList:
        strg = strg+"<TR ><td></td><TD>" + \
            str(itm.item.ItemDesc)+"</TD><TD>"+str(itm.issueDate) + \
            "</TD><TD>"+str(itm.ReqQty)+"</TD>"
        strg = strg+"<TD><div class=\"col-12\"><a href=" + \
            url_for('retUpdt', tid=itm.reqID)+">Return</a></div></TD></TR>"
    strg = strg+"</TABLE></div></FORM>"
...
def saveRequest():
    ...
    rItems = request_tbl(ReqDeptt=dept, ReqDate=rDate,
                         ItemID=iID, ReqQty=rQty, userID=uid)
